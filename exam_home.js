const PA_EXAM_HOME=(()=>{
  ...
  function pair(){
    const today=PA_EXAM_SUPPORT.day(),old=read(KEY),reads=loadDailyReadIds(),lit=literacy(),list=ids();
  ...
  function update(){
    const id=pair(),reads=loadDailyReadIds(),lit=literacy();
  ...
  async function prepare(){await loadDailyIndexJsonp();PA_PROGRESS.sync();return pair();}
