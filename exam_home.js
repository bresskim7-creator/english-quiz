const PA_EXAM_HOME=(()=>{
  ...
  function update(){
    ...
    for(const subject of ['english','science']){
      ...
      document.getElementById(subject+'-support-status').textContent=p?.completed&&(p.completed_day===PA_EXAM_SUPPORT.day()||p.date===PA_EXAM_SUPPORT.day())?'오늘 5문제 완료':subject==='english'&&state.completed_days>=3?'첫 3학습일 완료 · 다음 분량 준비 중':p&&!p.completed?`${p.answers.length}/5 · 이어 하기`:'하루 5문제';
    }
