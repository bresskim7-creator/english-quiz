const PA_EXAM_HOME=(()=>{
  ...
  function update(){
    ...
    for(const subject of ['english','science']){
      ...
      document.getElementById(subject+'-support-status').textContent=p?.completed&&!p.finalized?'5문제 풀이 끝 · 마무리하기':p?.completed&&(p.completed_day===PA_EXAM_SUPPORT.day()||p.date===PA_EXAM_SUPPORT.day())?'✓ 학습 완료':p&&!p.completed?`${p.answers.length}/5 · 이어 하기`:subject==='english'&&window.PA_ENGLISH_EXAM?.launch_days?.length&&state.completed_days>=window.PA_ENGLISH_EXAM.launch_days.length?`준비된 ${window.PA_ENGLISH_EXAM.launch_days.length}학습일 완료 · 다음 분량 준비 중`:'하루 5문제';
    }
